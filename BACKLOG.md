...
## ✅ Completado
- [x] Sistema de archivado de recetas: filtro persistente por pestañas (Activas/Archivadas/Todas) con chips, badge 📦 en tarjetas archivadas, acción de archivar/desarchivar desde el menú contextual `•••` y el modal de resumen, toast de "Deshacer" con auto-cierre de 4s, migración retrocompatible para recetas legacy sin `is_archived`, preservación del estado al editar/importar/exportar, iconos SVG (`ArchiveIcon`, `ArchiveRestoreIcon`, `InboxEmptyIcon`), estados vacíos contextuales, contadores animados y persistencia en localStorage (`coffee_recipe_filter_mode_v1`) (`v1.15.0`).
