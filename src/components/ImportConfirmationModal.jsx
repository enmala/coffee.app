    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-fade-in">
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto pr-1 space-y-4 text-left">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <div className="p-3 bg-zinc-50 dark:bg-zinc-800/40 rounded-xl border border-zinc-150 dark:border-zinc-800/80">
              <span className="block text-[10px] text-zinc-400 dark:text-zinc-500 font-semibold uppercase">Café</span>
              <span className="text-sm font-bold text-zinc-800 dark:text-zinc-200">{recipe.coffee_g}g</span>
            </div>
            <div className="p-3 bg-zinc-50 dark:bg-zinc-800/40 rounded-xl border border-zinc-150 dark:border-zinc-800/80">
              <span className="block text-[10px] text-zinc-400 dark:text-zinc-500 font-semibold uppercase">Molienda</span>
              <span className="text-sm font-bold text-zinc-800 dark:text-zinc-200 truncate block">{recipe.grind_size || 'N/A'}</span>
            </div>
            <div className="p-3 bg-zinc-50 dark:bg-zinc-800/40 rounded-xl border border-zinc-150 dark:border-zinc-800/80">
              <span className="block text-[10px] text-zinc-400 dark:text-zinc-500 font-semibold uppercase">Temperatura</span>
              <span className="text-sm font-bold text-zinc-800 dark:text-zinc-200">{recipe.water_temp_c || 'N/A'}°C</span>
            </div>
            <div className="p-3 bg-zinc-50 dark:bg-zinc-800/40 rounded-xl border border-zinc-150 dark:border-zinc-800/80">
              <span className="block text-[10px] text-zinc-400 dark:text-zinc-500 font-semibold uppercase">Agua Total</span>
              <span className="text-sm font-bold text-zinc-800 dark:text-zinc-200">{totalWater}g</span>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="border border-zinc-150 dark:border-zinc-800 rounded-xl overflow-hidden divide-y divide-zinc-150 dark:divide-zinc-800">
              {recipe.steps.map((step, index) => (
                <div key={index} className="p-3 hover:bg-zinc-50 dark:hover:bg-zinc-800/30 flex justify-between gap-4 transition-colors">
                  <div className="space-y-0.5 min-w-0">
                    <span className="inline-block text-[10px] bg-zinc-100 dark:bg-zinc-800 text-zinc-500 dark:text-zinc-400 font-bold px-1.5 py-0.5 rounded mr-2">
                      #{step.step_number}
                    </span>
                    <span className="font-bold text-zinc-800 dark:text-zinc-200 text-sm">{step.title}</span>
                    {step.instruction && (
                      <p className="text-xs text-zinc-500 dark:text-zinc-400 truncate max-w-xs pl-8">
                        {step.instruction}
                      </p>
                    )}
                  </div>
                  <div className="text-right shrink-0 flex flex-col justify-center text-xs font-semibold text-zinc-600 dark:text-zinc-400">
        ...
        <div className="flex gap-3 mt-6 pt-4 border-t border-zinc-150 dark:border-zinc-800 shrink-0">
          <button
            onClick={onCancel}
            className="flex-1 py-3 px-4 bg-zinc-100 dark:bg-zinc-800 hover:bg-zinc-200 dark:hover:bg-zinc-700 text-zinc-700 dark:text-zinc-300 font-bold rounded-xl active:scale-[0.98] transition-all text-center"
          >
            Cancelar
          </button>
          <button
