      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl max-w-sm w-full shadow-xl max-h-[85vh] border border-slate-100 dark:border-slate-800 flex flex-col overflow-hidden text-left">
        ...
        <div className="p-5 pb-3 border-b border-slate-200 dark:border-slate-800 shrink-0 bg-white dark:bg-slate-900 space-y-2">
          ...
          <div className="flex justify-between items-center pt-1">
            ...
            {!isLibraryPreview && summaryRecipe.is_archived && (
              <span className="inline-block text-[10px] bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-300 border border-slate-300 dark:border-slate-600 px-2 py-0.5 rounded font-bold uppercase tracking-wider ml-2 flex items-center gap-1">
                <ArchiveIcon className="w-3 h-3" /> Archivada
              </span>
            )}
        ...
        <div className="p-5 pt-3 border-t border-slate-100 dark:border-slate-800 flex gap-2 shrink-0 bg-white dark:bg-slate-900 items-center">
          {isLibraryPreview ? (
            ...
            <>
              ...
              {summaryRecipe.is_archived ? (
                <button
                  onClick={() => onUnarchiveRecipe && onUnarchiveRecipe(summaryRecipe.id)}
                  className="px-3 py-2 border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-200 rounded-xl font-bold text-xs transition cursor-pointer flex items-center justify-center gap-1.5"
                  title="Desarchivar receta"
                  aria-label="Desarchivar receta"
                >
                  <ArchiveRestoreIcon className="w-4 h-4" /> Desarchivar
                </button>
              ) : (
                <button
                  onClick={() => onArchiveRecipe && onArchiveRecipe(summaryRecipe.id, summaryRecipe.name)}
                  className="px-3 py-2 border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-200 rounded-xl font-bold text-xs transition cursor-pointer flex items-center justify-center gap-1.5"
                  title="Archivar receta"
                  aria-label="Archivar receta"
                >
