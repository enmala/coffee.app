...
## ✅ Completado
- [x] Importación unificada de recetas y granos de café desde el portapapeles: integración de la API del portapapeles (`navigator.clipboard.readText`) para importar la misma estructura JSON que los archivos `.json`, unificación del motor de validación y detección automática de entidad en `useNavigation`, rediseño de la sección de importación en el modal de configuración con botones duales e iconografía SVG (`DocumentTextIcon`, `ClipboardIcon`), gestión de errores de formato/permisos y suite de pruebas unitarias (`__tests__/clipboardImport.test.jsx`) (`v1.16.0`).
- [x] Sistema de archivado de recetas: filtro persistente por pestañas (Activas/Archivadas/Todas) con chips siempre visibles, badge 📦 en tarjetas archivadas, acción de archivar/desarchivar desde el menú contextual `•••` y el modal de resumen, toast de "Deshacer" con auto-cierre de 4s, migración retrocompatible para recetas legacy sin `is_archived`, preservación del estado al editar/importar/exportar, iconos SVG (`ArchiveIcon`, `ArchiveRestoreIcon`, `InboxEmptyIcon`), estados vacíos contextuales, contadores animados y persistencia en localStorage (`coffee_recipe_filter_mode_v1`). Fix: chips siempre visibles tras desarchivar todas las recetas + layout responsive móvil (`v1.15.0`).
- [x] Mejora de cobertura de pruebas para `coffeeUtils.jsx`: polyfill de `Blob.prototype.stream` en `__tests__/setup.js` para habilitar el path nativo de `CompressionStream`/`DecompressionStream` en jsdom, eliminando warnings (`TypeError: (intermediate value).stream is not a function`); tests para paths de error (`playBeep` catch, `speakText` con `speechSynthesis` no disponible, `speakText` catch, `getMethodIcon` con method falsy, defaults en `decompressRecipe`) (`v1.14.1`).
- [x] Mejora de cobertura de pruebas para `useRecipeLibrary.js`: nuevo archivo `__tests__/useRecipeLibrary.test.jsx` con 18 tests unitarios usando `renderHook` cubriendo paths de error (HTTP error, catálogo inválido, fallo de red sin caché, modo offline con caché), `reloadCatalog`, `methodsList`/`filteredRecipes`/`isRecipeImported` y race condition guards (`ignore` flag) (`v1.14.1`).
- [x] Actualización de tests de `ShareModal` y `ShareBeanModal` para esperar correctamente a la finalización asíncrona de la compresión (`waitFor` en botones de acción) tras la activación del path nativo de `CompressionStream` (`v1.14.1`).
- [x] Rediseño ergonómico del editor de pasos en recetas (`StepFormModal.jsx`): reemplazo del input de una línea por un `textarea` multilínea con autoajuste de texto para móviles, incorporación de chips de sugerencias rápidas no intrusivos (`STEP_INSTRUCTION_SUGGESTIONS`), modularización del componente desde `App.jsx` y etiquetas consistentes en todos los campos (`v1.14.0`).
- [x] Corrección de superposición z-index y transparencia en menú contextual de `RecipesTab`: mover el dropdown fuera del contenedor `opacity-85` para igualar comportamiento visual con `BeansTab` (`v1.13.1`).
